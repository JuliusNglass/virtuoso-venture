            ...
            {SLIDES.map((s, i) => (
              ...
              >
                <span className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full text-[10px] font-bold ${
                  i === current ? "bg-primary-foreground/20 text-primary-foreground" : "bg-muted-foreground/20"
                }`}>{s.id}</span>
              ...
              {SLIDES.map((s, i) => (
                <button
                  ...
                  className={`shrink-0 rounded-lg px-2.5 py-1.5 text-xs font-medium transition-all border ${
                    i === current ? "bg-primary text-primary-foreground border-primary" : "bg-background text-muted-foreground border-border"
                  }`}
                >
                  {s.id}. {s.title}
                </button>
